...
def calculate_paths(pdb, num, cutoff_num, total_paths, type_da, alpha_reorg, dir_path):
    ...
    try:
        ...
        if False:
            ...
            # remove certain atoms that do not contain significant charge density. For example, we exclude the peptide backbone for the amino acids, and the sugar phosphate backbond for nucleic acids.
            ...
            ##############Donor-Bridge##############
                ...
                #For 1RRQ: Compute Marcus parameters between the *hole* donor node (G/A) and a bridge node (amino acid) for the forward *HT* step.     
                if type_da=='hole' and (source_node.nodeNum in donor_list and source_node.Residuetype in ['GTP','G','DG','A','DA','gtp','DG5','8OG','DA3','DA5']) and (target_node.nodeNum in bridge_list and target_node.Residuetype in ['CYS','MET','TRP','TYR']):
                    Lambda=(target_node.Lamb/27.2114+0.5*(1/2.2-1/4)*(mean_distance-12.8)/(mean_distance*12.8)/1.889726)+(source_node.Lamb/27.2114+0.5*(1/2.2-1/4)*(1/source_node.r-1/mean_distance)*(1/1.889726))
                ...
                #For 1RRQ: Compute Marcus parameters between the *hole* donor node (G/A) and a bridge node (amino acid) for the backward *HT* step.     
                if type_da=='hole' and (source_node.nodeNum in bridge_list and source_node.Residuetype in ['CYS','MET','TRP','TYR']) and (target_node.nodeNum in donor_list and target_node.Residuetype in ['GTP','G','DG','A','DA','gtp','DG5','8OG','DA3','DA5']):
                ...
                #For 1RRQ: Compute Marcus parameters between the *hole* donor node (G/A) and a bridge node (DAB) for the forward *ET* step.     
                if type_da=='hole' and (source_node.nodeNum in donor_list and source_node.Residuetype in ['GTP','G','DG','A','DA','gtp','DG5','8OG','DA3','DA5']) and (target_node.nodeNum in bridge_list and target_node.Residuetype in ['DAB']):
                ...
                if type_da=='hole' and (source_node.nodeNum in bridge_list and source_node.Residuetype in ['DAB']) and (target_node.nodeNum in donor_list and target_node.Residuetype in ['GTP','G','DG','A','DA','gtp','DG5','8OG','DA3','DA5']):
                    Lambda=(target_node.Lamb/27.2114+0.5*(1/2.2-1/4)*(1/target_node.r-1/mean_distance)*(1/1.889726))+(source_node.Lamb/27.2114+0.5*(1/2.2-1/4)*(1/source_node.r-1/mean_distance)*(1/1.889726))
                    Lambda=0.8+alpha_reorg*(Lambda*27.2114-0.8)
                    deltaG=target_node.E-source_node.E
                    VIF=2.7/(source_node.AtomQuant*target_node.AtomQuant)**0.5 * np.exp(-0.72*shortest_distance)
            ...
            ##############Bridge-Acceptor##############
                ...
                #For 1RRQ: Compute Marcus parameters between a bridge node (amino acid) and the terminal *hole* acceptor node ([4Fe4S]) for the forward *HT* step.     
                ...
                #For 1RRQ: Compute Marcus parameters between a bridge node (DAB) and the terminal *hole* acceptor node ([4Fe4S]) for the forward *HT* step.     
                if type_da=='hole' and (source_node.nodeNum in bridge_list and source_node.Residuetype in ['DAB']) and (target_node.nodeNum in acceptor_list and target_node.Residuetype in ['SF4','FS4']):
            ...
            ##############Donor-Acceptor##############
                ...
                #For 1RRQ: Compute Marcus parameters between the *hole* donor node (G/A) and the terminal *hole* acceptor node ([4Fe4S]) for the forward *HT* step.     
                if type_da=='hole' and (source_node.nodeNum in donor_list and source_node.Residuetype in ['GTP','G','DG','A','DA','gtp','DG5','8OG','DA3','DA5']) and (target_node.nodeNum in acceptor_list and target_node.Residuetype in ['SF4','FS4']):
                    Lambda=(target_node.Lamb/27.2114+0.5*(1/2.2-1/4)*(mean_distance-14.44)/(mean_distance*14.44)/1.889726)+(source_node.Lamb/27.2114+0.5*(1/2.2-1/4)*(1/source_node.r-1/mean_distance)*(1/1.889726))
                    Lambda=0.8+alpha_reorg*(Lambda*27.2114-0.8)
                    deltaG=target_node.E-source_node.E
            ...
            ##############Bridge-Bridge##############
                ...
                if type_da=='hole' and (source_node.nodeNum in bridge_list and source_node.Residuetype in ['DAB']) and (target_node.nodeNum in bridge_list and target_node.Residuetype in ['CYS','MET','TRP','TYR']):
                ...
                if type_da=='hole' and (target_node.nodeNum in bridge_list and target_node.Residuetype in ['DAB']) and (source_node.nodeNum in bridge_list and source_node.Residuetype in ['CYS','MET','TRP','TYR']):
